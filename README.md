...
## Results (run 1, 2 → 6 September 2026)

400k steps, 102M samples (24 epochs of 4.2M images), 3.5 days on one GPU. Every image below comes from the
final EMA weights with fixed seeds and no cherry-picking within a set; the prompt grids use 50 Euler steps,
CFG 4, shift 2.8 (`scripts/make_readme_figures.py --steps 50`), the metrics are measured with the 20-step
training-time sampler. 50 steps versus 20 is worth 0.4 FID and nothing on the preference scores (table below).
...
**Inference settings**, same prompt and seed. Guidance matters most: CFG 1 loses the prompt, CFG 7
over-saturates. Measured on the 2,456 held-out prompts with the final weights:
...
The shift the model was trained with is worth more than doubling the step count; beyond 20 steps the
returns are small, which the trajectory figure above explains.
